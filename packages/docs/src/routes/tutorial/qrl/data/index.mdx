...
For this exercise imagine that we wish to lazy load the `'Hello World!'` string. Use the `$()` function to mark the string as lazy-loadable. Then call `.resolve()` on that to get a Promise for the lazy-loadable value and then use `await` to obtain the value.
